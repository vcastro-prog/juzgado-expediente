...
from parser_pdf import extraer_expedientes, extraer_diagnostico_pdf, PARSER_VERSION
try:
...
st.markdown(
    ...
    <div style="display:flex; align-items:baseline; gap:14px; margin-bottom:0.4rem;">
        ...
        <span style="font-size:0.95rem; color:gray; white-space:nowrap;">
            App {APP_VERSION} · Build {BUILD_DATE} · {PARSER_VERSION}
        </span>
    </div>
    """,
...
with st.sidebar:
    ...
    if pdf is not None:
        if st.button("Procesar PDF", type="primary"):
            ...
            st.caption(
                f"Diagnóstico · Tipo: {diagnostico['tipo_pdf']} · "
                f"Páginas: {diagnostico['paginas']} · "
                f"Registros detectados: {diagnostico['registros_detectados']} · "
                f"{diagnostico['parser_version']}"
            )

            if diagnostico.get("error"):
                st.error(diagnostico["error"])
            else:
